...
def load_from_API():
    # Was ist gesucht - API schlussel (in .env File), suchen nach donald trump, alle Bildtypen und nur 10 Antworten
    parameters = {
        'key': keyinfo,
        'q': 'donald+trump',
        'image_type': 'all',
    ...
    # Abholung von den Bilderinfo und Addressen bie pixabay
...
def load_up_choice():
    if "API_data" in session:
        return render_template('demo.html', respdon=session["API_data"][0], respput=session["API_data"][1],
                               respangie=session["API_data"][2])
    else:
...
def displ():
    if request.method == 'POST':
        if "submit-img" in request.form:
            ...
            return render_template('display.html', jsoneddata=jsoneddata, topText="", bottomText="",
                                   textColor="black", textSize="30", displayed=displayed)
        elif "apply" in request.form:
            # Text Data in Bild verbunden
            displayed = True
            return render_template('display.html', jsoneddata=session['picdata'], topText=request.form["topText"],
                                   bottomText=request.form["bottomText"], textColor=request.form["textColor"],
                                   textSize=request.form["textSize"], displayed=displayed)
